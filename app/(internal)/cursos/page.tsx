...
export default async function CursosPage() {
    ...
    const { data: cursos, error } = await query.order('created_at', { ascending: false })

    if (error) {
        return (
            <div className="p-8 text-center bg-orange-50 text-orange-700 rounded-lg">
                <h2 className="font-bold text-lg mb-2">Erro ao carregar cursos</h2>
                <p className="text-sm">{error.message}</p>
            </div>
        )
    }

    return <CursosTable initialData={(cursos as any[]) || []} />
}
