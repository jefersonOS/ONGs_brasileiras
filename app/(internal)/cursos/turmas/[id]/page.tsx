...
export default function TurmasPage({ params }: { params: { id: string } }) {
    ...
    const handleEdit = (turma: any) => {
        ...
        setDataLimiteInscricao(turma.data_limite_inscricao || '')
        setFormularioInscricao(turma.formulario_inscricao || CAMPOS_DEFAULT)
        setMostrarForm(true)
    ...
    return (
        <div className="space-y-6">
            ...
            {mostrarForm && (
                <form onSubmit={handleSaveTurma} className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm space-y-8">
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                        <button type="button" onClick={() => { setMostrarForm(false); setEditingTurmaId(null) }} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">Cancelar</button>
                        <button type="submit" disabled={loading} className="px-4 py-2 text-sm bg-[#1A3C4A] text-white rounded-md hover:bg-[#2E6B7A] disabled:opacity-50">
